...
export default function AppBottomTabs({ page }) {
  return (
    <nav className="fixed bottom-0 left-1/2 z-50 flex w-[min(390px,calc(100%-12px))] -translate-x-1/2 items-center justify-between rounded-t-lg border border-mint-100 bg-white px-1.5 py-1.5 shadow-[0_-6px_20px_rgba(0,0,0,0.06)]">
      {tabs.map((tab) => {
        const Icon = tab.icon
        ...
        return (
          <a
            ...
            className={`flex min-w-0 flex-1 flex-col items-center gap-0.5 rounded-md px-1 py-0.5 no-underline transition ${
              active ? 'text-mint-500' : 'text-bluegray-500 hover:bg-mint-50'
            }`}
          >
